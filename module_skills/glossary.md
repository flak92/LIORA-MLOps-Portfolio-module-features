...
code and the mounts carry the stores each service touches, read-only where it
...
| the workspace: a recursive clone of Orchestration with the four module repositories checked out inside it | `git clone --recurse-submodules`; the checkouts `901-module_data/`, `902-module_features/`, `903-module_ml/`, `904-module_monitoring/` — `<9NN>-module_<domain>/`, the number the module's position in the chain at the time it was seated, never renumbered; the four listed as `SUBMODULES` in the Makefile, the list `build` and the `skills-*` targets iterate | `.gitmodules` (`path`, `url`, `branch = main`) | — | a renumbered checkout; a submodule on a detached HEAD or on another branch; a monorepo |
...
|---|---|---|
| `MILLISECONDS_PER_SECOND`, `MILLISECONDS_PER_MINUTE`, `MILLISECONDS_PER_DAY` (each module the ones it uses) | `module_data/config.py`, `module_features/config.py`, `module_ml/config.py`; the browser's own in `module_monitoring/page.js` and `module_monitoring/sub_module_devops/containers.js`, which import no config | a unit is a unit; importing one across a boundary would drag the module behind it |
